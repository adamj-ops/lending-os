...
5. Copy the "Publishable Key" → `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`
...
curl http://localhost:3000/api/v1/loans \
  # Clerk handles authentication automatically via middleware
```
...
Check that:
1. `CLERK_SECRET_KEY` and `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` are set in `.env.local`
