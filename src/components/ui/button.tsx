...
const buttonVariants = cva(
  'cursor-pointer group whitespace-nowrap inline-flex items-center justify-center has-data-[arrow=true]:justify-between text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
    ...
    compoundVariants: [
      // Icons opacity for default mode
      {
        variant: 'ghost',
        mode: 'default',
        className: '[&_svg:not([role=img]):not([class*=text-]):not([class*=opacity-])]:opacity-70',
      },
      {
        variant: 'outline',
        mode: 'default',
      ...
      // Icons opacity for input/icon modes
      {
        variant: 'outline',
        mode: 'input',
        className: '[&_svg:not([role=img]):not([class*=text-]):not([class*=opacity-])]:opacity-70',
...
interface ButtonArrowProps extends React.SVGProps<SVGSVGElement> {
  icon?: Icon; // Allows passing any Tabler icon
}

function ButtonArrow({ icon: Icon = IconChevronDown, className, ...props }: ButtonArrowProps) {
