...
import { IconArrowUpRight, IconMinus, IconDotsVertical, IconPin, IconSettings, IconShare, IconShieldCheck, IconAlertTriangle } from "@tabler/icons-react";
...
function GrowthBadge({ direction, growth }: { direction: string; growth: number }) {
  ...
  return (
    <Badge className="bg-muted text-muted-foreground px-2 py-0.5 rounded text-xs font-medium flex items-center gap-1">
      <IconMinus size={12} stroke={2} className="w-3 h-3" />
      {growth}%
    </Badge>
  );
}
          ...
          <CardToolbar>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" side="bottom">
                ...
                <DropdownMenuItem>
                  <IconAlertTriangle size={16} stroke={2} /> Export Report
                </DropdownMenuItem>
                <DropdownMenuItem>
                  <IconPin size={16} stroke={2} /> Configure Alerts
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem>
                  <IconShare size={16} stroke={2} /> Run Manual Check
                </DropdownMenuItem>
                <DropdownMenuItem>
                  <IconShieldCheck size={16} stroke={2} /> View History
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </CardToolbar>
