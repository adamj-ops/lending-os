...
function EventItem({ event }: EventItemProps) {
  ...
  const getEventColor = (eventType: string) => {
    if (eventType.includes('Late') || eventType.includes('Failed')) return 'text-brand-danger';
    if (eventType.includes('Completed') || eventType.includes('Received')) return 'text-brand-success';
    if (eventType.includes('Pending') || eventType.includes('Scheduled')) return 'text-brand-accent';
      ...
      <CardContent>
        {error && (
          <div className="p-4 border border-red-600 rounded-lg bg-transparent dark:border-red-400">
            <p className="text-sm text-brand-danger dark:text-red-400">Error: {error}</p>
          </div>
        )}
