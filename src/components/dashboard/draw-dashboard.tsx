...
export function DrawDashboard({ loanId, className, viewOnly = false }: DrawDashboardProps) {
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'approval': return <IconCircleCheck size={20} stroke={2} className="h-4 w-4 text-green-600" />;
      case 'inspection': return <IconCalendar size={20} stroke={2} className="h-4 w-4 text-yellow-600" />;
      case 'rejection': return <IconAlertCircle size={20} stroke={2} className="h-4 w-4 text-red-600" />;
      default: return <IconActivity size={20} stroke={2} className="h-4 w-4 text-gray-600" />;
    }
  };
