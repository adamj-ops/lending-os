...
const InspectionScheduler = ({ loanId, onInspectionScheduled }: any) => (
  <Card>
    <CardContent className="text-center py-8">
      <p className="text-brand-muted">InspectionScheduler Component (TODO)</p>
      <p className="text-sm mt-2 text-brand-muted">This component will allow scheduling new inspections</p>
    </CardContent>
  </Card>
);
              ...
              <div key={workload.inspectorId} className="border rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-brand-muted">Inspector ID: {workload.inspectorId}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-4 mb-3">
                  <div className="text-center">
                    <div className="text-lg font-bold text-brand-success">{workload.completedInspections}</div>
                    <div className="text-xs text-brand-muted">Completed</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-brand-accent">{workload.pendingInspections}</div>
                    <div className="text-xs text-brand-muted">Pending</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-brand-danger">{workload.overdueInspections}</div>
                    <div className="text-xs text-brand-muted">Overdue</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-blue-600">{workload.avgCompletionTime.toFixed(1)}h</div>
                    <div className="text-xs text-brand-muted">Avg Time</div>
                  </div>
                </div>
                ...
                <div key={inspection.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-gray-100 rounded-lg">
                      <IconMapPin size={20} stroke={2} className="h-4 w-4 text-brand-muted" />
                    </div>
                    <div>
                      <h4 className="font-medium text-sm">Inspection #{inspection.id.slice(-6)}</h4>
                      <p className="text-xs text-brand-muted">{inspection.drawId}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge className={
