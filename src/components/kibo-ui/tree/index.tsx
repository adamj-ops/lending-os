...
export const TreeExpander = ({
  ...
  return (
    <motion.div
      ...
      {...props}
    >
      <IconChevronRight size={20} stroke={2} className="h-3 w-3 text-muted-foreground" />
    </motion.div>
  );
};
...
export const TreeIcon = ({
  ...
  const getDefaultIcon = () =>
    hasChildren ? (
      isExpanded ? (
        <IconFolderOpen size={16} stroke={2} className="h-4 w-4" />
      ) : (
        <IconFolder size={20} stroke={2} className="h-4 w-4" />
      )
    ) : (
      <IconFile size={20} stroke={2} className="h-4 w-4" />
    );
